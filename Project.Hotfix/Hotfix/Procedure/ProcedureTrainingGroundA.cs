...
{	
	//场景训练A
	public class ProcedureTrainingGroundA : ProcedureLogic
	{
		...
		public override void OnUpdate(IFsm<IProcedureManager> procedureOwner)
		{
			...
			//	GameEntry.Event.Fire(this, ProcedureChangedEventArgs.Fill(GameEntry.Config.GetInt("Scene.TrainingA")));
			//}
			if (UnityEngine.Input.GetKeyDown(UnityEngine.KeyCode.B))
			{
				HotLog.Debug("切换场景=>训练场景B");
				HotfixEntry.Event.Fire(this, ProcedureChangedEventArgs.Fill(GameEntry.Config.GetInt("Scene.TrainingB")));
			}
			if (isChangeScene)
			{
				isChangeScene = false;
