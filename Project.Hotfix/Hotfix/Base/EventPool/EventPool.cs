...
using ReferencePool = Project.Hotfix.Reference.ReferencePool;
...
namespace Project.Hotfix.Event
{
    ...
    internal sealed partial class EventPool<T> where T : BaseEventArgs
    {
        ...
        public void Subscribe(int id, EventHandler<T> handler)
        {
            ...
            {
                throw new Exception(GameFramework.Utility.Text.Format("Event '{0}' not allow multi handler.", id.ToString()));
            }
            else if ((m_EventPoolMode & EventPoolMode.AllowDuplicateHandler) != EventPoolMode.AllowDuplicateHandler && Check(id, handler))
            {
                throw new Exception(GameFramework.Utility.Text.Format("Event '{0}' not allow duplicate handler.", id.ToString()));
            }
            else
            {
                m_EventHandlers.Add(id, handler);
        ...
        public void Unsubscribe(int id, EventHandler<T> handler)
        {
            ...
            {
                throw new Exception(GameFramework.Utility.Text.Format("Event '{0}' not exists specified handler.", id.ToString()));
            }
        }
        ...
        private void HandleEvent(object sender, T e)
        {
            ...
            {
                throw new Exception(GameFramework.Utility.Text.Format("Event '{0}' not allow no handler.", e.Id.ToString()));
            }
        }
    }
}
