...
namespace Project.Hotfix.ObjectPool
{
    internal sealed partial class ObjectPoolManager : IObjectPoolManager
    {
        ...
        {
            private readonly GameFramework.GameFrameworkMultiDictionary<string, Object<T>> m_Objects;
            ...
            public ObjectPool(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority)
                : base(name)
            {
                m_Objects = new GameFramework.GameFrameworkMultiDictionary<string, Object<T>>();
            ...
            public bool CanSpawn(string name)
            {
                ...
                GameFramework.GameFrameworkLinkedListRange<Object<T>> objectRange = default(GameFramework.GameFrameworkLinkedListRange<Object<T>>);
            ...
            public void Unspawn(object target)
            {
                ...
                {
                    throw new Exception(GameFramework.Utility.Text.Format("Can not find target in object pool '{0}', target type is '{1}', target value is '{2}'.", new TypeNamePair(typeof(T), Name).ToString(), target.GetType().FullName, target.ToString()));
                }
            }
            ...
            public override ObjectInfo[] GetAllObjectInfos()
            {
                ...
                foreach (KeyValuePair<string, GameFramework.GameFrameworkLinkedListRange<Object<T>>> objectRanges in m_Objects)
                {
