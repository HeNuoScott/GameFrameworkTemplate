...
namespace Sirius.Runtime
{
    public static class SoundExtension
    {
        ...
        public static int? PlayMusic(this SoundComponent soundComponent, string musicName, object userData = null)
        {
            soundComponent.StopMusic();

            IDataTable<DRMusic> dtMusic = GameEntry.DataTable.GetDataTable<DRMusic>();
            DRMusic drMusic = dtMusic.GetDataRow((musicData) => { return musicData.AssetName == musicName ? true : false; });
            if (drMusic == null)
            {
                Log.Warning("Can not load music '{0}' from data table.", musicName);
                return null;
            }
        ...
        public static int? PlaySound(this SoundComponent soundComponent, int soundId, Entity bindingEntity = null, object userData = null)
        {
            ...
            return soundComponent.PlaySound(AssetUtility.GetSoundAsset(drSound.AssetName), "Sound", Constant.AssetPriority.SoundAsset,
                playSoundParams, bindingEntity != null ? bindingEntity.Entity : null, userData);
        }

        public static int? PlaySound(this SoundComponent soundComponent, string soundName, Entity bindingEntity = null, object userData = null)
        {
            IDataTable<DRSound> dtSound = GameEntry.DataTable.GetDataTable<DRSound>();
            DRSound drSound = dtSound.GetDataRow((soundData) => { return soundData.AssetName == soundName ? true : false; });
            if (drSound == null)
            {
                Log.Warning("Can not load sound '{0}' from data table.", soundName);
                return null;
            }
            ...
            playSoundParams.VolumeInSoundGroup = drSound.Volume;
            playSoundParams.SpatialBlend = drSound.SpatialBlend;
            playSoundParams.MaxDistance = drSound.MaxDistance;
        ...
        public static int? PlayUISound(this SoundComponent soundComponent, string uiSoundName, object userData = null)
        {
            IDataTable<DRUISound> dtUISound = GameEntry.DataTable.GetDataTable<DRUISound>();
            DRUISound drUISound = dtUISound.GetDataRow((soundData) => { return soundData.AssetName == uiSoundName ? true : false; });
            if (drUISound == null)
            {
                Log.Warning("Can not load UI sound '{0}' from data table.", uiSoundName);
                return null;
            }
            PlaySoundParams playSoundParams = PlaySoundParams.Create();
            playSoundParams.Priority = drUISound.Priority;
