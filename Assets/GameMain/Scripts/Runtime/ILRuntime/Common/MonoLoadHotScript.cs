...
public class MonoLoadHotScript : MonoBehaviour
{
    ...
    private void Start()
    {
        InitLogic(HotLogicScript);
    }
    public void InitLogic(string HotLogicTypeName)
    {
        ReferenceCollector = GetComponent<ReferenceCollector>();
        HotLogicTypeName = HotLogicTypeName.HotFixTypeFullName();
        HotLogicInstance = GameEntry.Hotfix.CreateInstance(HotLogicTypeName, null);  //创建实例
